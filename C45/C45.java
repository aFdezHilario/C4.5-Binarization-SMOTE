...
public class C45
   ...
   * @throws Exception	If the algorithm cannot be executed.
   */
  public C45(String fichTrain, boolean pruned, float confidence, int instancesPerLeaf) {
    modelDataset = new Dataset(fichTrain, false); //false?
    priorsProbabilities = new double[modelDataset.numClasses()];
    try {
      priorsProbabilities();
    }
  ...
  public C45(myDataset train, myDataset val, myDataset test) throws Exception {
		try {

			//prune = true;
			//confidence = 0.25;
			modelDataset = new Dataset(train);
			trainDataset = new Dataset(val);
			...
			//System.err.println("Se lia parda... "+e.getMessage());
			e.printStackTrace(System.err);
			System.exit( -1);
		}
	}
  ...
  public void priorsProbabilities() throws Exception {
	...
	public double [] probs(double [] example){
		double [] probs = new double[modelDataset.numClasses()];
		try {
			Itemset itemset = new Itemset(1, example);
			itemset.setDataset(modelDataset);
