...
public class SMOTE extends Metodo {
	...
	public void run () {
		...
		cl2 = 0;
		cl1 = clasesTrain[0];
		for (i=1; i<clasesTrain.length; i++) {
			if (cl1 != clasesTrain[i]){
				cl2 = clasesTrain[i];
				break;
			}
		}
		int [] clases_train_aux = clasesTrain.clone();
		for (i=0; i<clases_train_aux.length; i++) {
			if(clases_train_aux[i] == cl1){
				clases_train_aux[i] = 0;
			}else{
				clases_train_aux[i] = 1;
			}
		}
		int max_cl = cl1 > cl2? cl1+1 : cl2+1;
		
		for (i=0; i<clasesTrain.length; i++) {
			if (clasesTrain[i] == cl1)
				nPos++;
			else
				nNeg++;
		}
		...
		//System.err.println("Mira: "+cl1+"/"+cl2+": "+posID);
		...
		for (i=0; i<positives.length; i++) {
			...
			case 0:
				...
				KNN.evaluacionKNN2 (kSMOTE, datosTrain, realTrain, nominalTrain, nulosTrain, clases_train_aux, datosTrain[positives[i]], realTrain[positives[i]], nominalTrain[positives[i]], nulosTrain[positives[i]], max_cl, distanceEu, neighbors[i]);        		
				break;
			case 1:
				//Cambiar si funciona...
				evaluationKNNClass (kSMOTE, datosTrain, realTrain, nominalTrain, nulosTrain, clasesTrain, datosTrain[positives[i]], realTrain[positives[i]], nominalTrain[positives[i]], nulosTrain[positives[i]], max_cl, distanceEu, neighbors[i],posID);
				break;
			case 2:
				evaluationKNNClass (kSMOTE, datosTrain, realTrain, nominalTrain, nulosTrain, clasesTrain, datosTrain[positives[i]], realTrain[positives[i]], nominalTrain[positives[i]], nulosTrain[positives[i]], max_cl, distanceEu, neighbors[i],negID);
				break;
			}
		}
